...
chooseGrid x y context = do
	...
	let getRectCoord start end coord = if start <= coord && coord < end then Just coord else Nothing
	let pointToCorner (x, y) = do
...
-- main = blankCanvas 3000 $ animateGrid gosperGliderGun